...
- [x] **V-D1. `docs/PROJECT_PLAN.md` を現状に追随させる**（2026-07-25 起票・2026-08-22 完了）
      §7 マイルストーンは **v0.1 MVP 時点の gantt** のままで、v0.3 に「PDF/A・PDF/UA 準拠検証（範囲再検討）」が
      未着手として残っていた。§2 スコープ表の「v0.1 は XMP 宣言の**識別**のみ」も現状と合っていなかった。

      **改訂の形（2026-08-22・v0.17.0 時点）**: 実績で計画を上書きせず、**「計画」と
      「その後どうなったか」の並記**にした — 何を先送りにし、どこで判断を変えたかが追える
      ことがこの文書の残す価値のため。§2 スコープ表と §9 リスク表に実績列を追加、
      §3 は 7 ツール、§5 に normativepdf / pdf-constraints を追記、§7 の gantt は実績表 +
      「残タスクは TASKS.md が正典」に置換。日付・版数はすべて CHANGELOG から引いた。
      Issue #1 への反映文は起票済み（改訂コミット後に貼る）
