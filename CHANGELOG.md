...
PDF/A が埋め込みを求めるのは**描画に使われたフォント**である。
独立オラクル（veraPDF 1.30.2）と突き合わせて、106 件中 87 件で
こちらだけが違反と言っていたことが分かった。

### Changed

- **`validate_conformance` の PDF/A `fonts-embedded` が、フォントの使われ方を見る。**

  コンテンツストリームを演算子まで読み、`Tf` で選んだフォントが `Tj`/`TJ`/`'`/`"`
  するときのテキストレンダリングモード（ISO 32000-2 §9.3.6）を記録する。
  走る範囲はページのコンテンツ・フォーム XObject（深さ 4 まで）・
  注釈の外観ストリーム `/AP /N`（§12.5.5。フォーム欄の値はここにある）。

  ```
  fail         埋め込まれていないフォントが、モード 3 以外で文字を出している
  違反なし      すべて未使用かモード 3 だけ
  skippedRules 観測が完全でない（読めない・深すぎる・未対応の経路）
  ```

  **判定していない規則は pass にも violation にも数えない** ——
  PDF/UA 側の `skippedRules`（Issue #7）と同じ扱いをここでも使う。
  `RuleResult` に `checked` が増えた。

  **走っていない経路**（`skippedRules` に落ちる）: Type3 の CharProcs・
  タイリングパターン・ExtGState の `/SMask` が指すグループ・深さ 4 より深いフォーム。

- 回復方針の 3 ファイルを `@normativepdf/recover` に切り出した（0.22.0）ぶんの
  依存を **0.1.1** に上げた。`has()` が ISO 32000-2 §7.3.7
  「値が `null` のエントリは、エントリが存在しないのと同じものとして扱う」を
  当てていなかったのを直した版である。`/FontFile3 null` の文書に
  「埋め込まれている」と答えていた。
...
```
recover 0.1.0 -> 0.1.1（has だけ）
  差 2 件。どちらも veraPDF の 6-2-10-4-1-t01-{pass,fail}-a.pdf で
  /FontFile3 null を「埋め込まれていない」と読むようになったもの

fonts-embedded を使われ方で見る（0.24.0 の本体）
  差 105 件（validate_conformance のみ）。全件の帰属:
    87  fail -> 違反なし   probe がすべて「どこでも文字を出していない」と分類
    16  fail のまま・文言だけ変わった
     2  fail のまま・挙げるフォントが減った（描画していないフォントが落ちた）
  🔴 fail のまま残った 19 件は、probe が全件「モード 3 以外で出している」と分類。
     緩んだ 87 件に visible は 1 件も無い
  veraPDF が PASS と言う 6-2-10-4-1-t01-pass-a.pdf は、こちらも違反なしになった
```

markdown のゴールデンも同じ 105 件で一致。単体 198 件（+6）。
検体を 3 件足した（`.golden/specimens/nullentry-*.pdf`。同じ組み立てで
`/FontFile2` の値と `Tr` だけが違う 3 つ組）。

### Fixed

- 検体を作るときにオブジェクト番号を飛ばしていて、相互参照表の `0 N` が
  1 節で連番を前提にしているため `/Contents` が別のオブジェクトを指していた。
  検体が「文字を出していない」ことになり、狙った軸を測っていなかった。
...
最上位が配列から辞書になった。0.20.0 で始めた「7 ツールが同じことを言う」を
ここで完了する。

### Changed

- **BREAKING — `verify_signatures` の JSON**

  ```jsonc
  // 0.20.0 まで
  [ { "fieldName": "...", "verdict": "valid", ... } ]
  // 0.21.0
  { "scope": { ... }, "signatures": [ { "fieldName": "...", ... } ] }
  ```

- **BREAKING — `detect_pades_level` の JSON**: 同じ形で `{ scope, levels: [...] }`。

  **移行**: 配列を直接読んでいたなら `.signatures` / `.levels` を挟む。
  一覧の中身は 1 要素も 1 鍵も変わっていない —— 実測で、2,950 検体すべてにおいて
  包む前と包んだあとの一覧が**バイト単位で同一**である。

  **なぜ形を変えるのか**: 署名の一覧が「ファイルにある署名の全部」だとは限らない。
  相互参照表を組み直した文書（`scope.reconstructed`）では、組み直しに入らなかった
  署名は一覧に出ない。**一覧と同じ場所で射程を読めないと、欠けていることが
  分からない。** 0.20.0 は 5 本に `scope` を載せたが、いちばん要るこの 2 本が
  配列だったために残っていた。

- **markdown も射程を出す。署名が 0 本のときも出す。**
  「署名が無い」と「読めた範囲に署名が無い」は別で、後者は表を組み直した文書で
  起きる。0 本の分岐で射程を落とすと、その 2 つが同じ文になる。

### Notes

- **判定は 1 件も動いていない。** 2,950 検体 × 7 ツール = 20,650 呼び出しで A/B を
  採り、差 5,898 件を全件帰属した:
  - 差はすべて `verify_signatures` と `detect_pades_level`。**ほかの 5 本は 1 件も
    動いていない**
  - 5,898 件すべてで、抽出した判定（`kept`）が完全に同じで、一覧の中身が
    1 バイトも変わっていない
  - 25,000 字（`CHARACTER_LIMIT`）で切り詰められる呼び出しは 3 件のまま。増えていない
- 空振り検査の対は `node scripts/golden.mjs t3` の 14 件。
- 🔴 計器が 1 度誤報した。`golden.mjs` の抽出が最上位の配列しか読まず、
  一覧が変わっていないのに 48 件を「観測できた対象が減った」と分類した。
  両方の形を読むよう直してから採り直した（`kept` は take のときに確定する）。
- `engines.node` は変わらない: `>=20`。

### 追随（別リポジトリ・同日）

- `pdf-trust-skill` 0.8.0 —— 判定の前に射程を読む手順（Phase 1.5）と、
  **条文を名指しする `PARSE_FAILED` を「未実施項目」に入れない**規則
- `pdf-publish-skill` 0.6.0 —— 読み戻しで `reconstructed: true` なら
  **こちらの出力の欠陥**として扱う
...
**ツールの引数も判定の規則も変えていない。** 変わったのは 2 つ ——
報告に項目が 1 つ増えたことと、条文を名指しする失敗の `code` である。

### Added

- **7 ツールのうち 5 本が `scope` を返すようになった**（`verify_integrity` /
  `identify_conformance` / `validate_conformance` / `validate_clauses` /
  ...
  **verify が推測した表の上での違反なし**であって、ファイルが持っている表の
  上での話ではない。監査の読み手に伏せてよい事実ではないので載せた。
  実測で 2,950 検体のうち 7 件がこれに当たる。

  配列を返す `verify_signatures` と `detect_pades_level` は、最上位が配列なので
  `scope` を並べられない。**形の変更（破壊的）になるため 0.21.0 に分けた。**

### Changed

- **条文を名指しする失敗の `code` が `PARSE_FAILED` に揃った。**
  0.19.0 では `isError` 26 件すべてがメッセージで条文を名指ししているのに、
  20 件の `code` は `INTERNAL_ERROR` だった —— 「この文書は §7.5.4 に反する」を
  **サーバの故障として**返していた。受け側がこれを「取得失敗」に分類すると、
  条文違反が「調べられませんでした」になり、下手な文書ほど無罪になる。
  `suggestion` に「これはファイルについての所見であって、このサーバの失敗では
  ない」を書くようにした（27 件すべてに付く。以前は 6 件）。

  **`INTERNAL_ERROR` で分岐していた呼び出し側は、`PARSE_FAILED` を見ること。**
  `INTERNAL_ERROR` は本当に内部で落ちたときだけに戻った。

- **`sections` の意味が経路によらなくなった。** 0.19.0 まではライブラリが
  そのまま読んだ経路で節を数えておらず、**健全な文書がすべて `0`** を返して
  いた（2,947 件のうち 2,931 件）。読み手はそれを「節が 0 個」と読む。
  数えるようにし、数えられないときは `null` にした。`0` は「節が 1 つも
  読めなかった」という観測結果のときだけ使う。

### Notes

- **判定は 1 件も動いていない。** 検体 2,950 × ツール 7 = 20,650 呼び出しで
  A/B を採り、差 14,737 件を段ごとに帰属した:
  - S0 検体 3 件を足した（出力は動かない。共有する 2,947 件の差は 0 件）
  - S1 `sections`（出力は動かない。差 0 件）
  - S2 `scope` を載せた —— 差 14,725 件のうち **14,722 件が「鍵が増えただけ」**。
    残り 3 件は 25,000 字の上限で切り詰められる報告で、切り詰め後の文字列が
    変わったもの。**新たに読めなくなった呼び出しは無い**（切り詰めは前から同じ 3 件）
  - S4 `code` —— 差 27 件、**すべて `isError` の payload**。それ以外は動いていない
- 空振り検査の対は `node scripts/golden.mjs t3` の 14 件（各段で実施）。
- 検体を 3 件足した（`XrefChainStop` の 5 値のうち `cyclic` と `malformed` は
  0.19.0 の集合で 1 件も踏まれていなかった）。いま 5 値すべてが出る。
- `engines.node` は変わらない: `>=20`。
