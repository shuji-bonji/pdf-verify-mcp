...
| `verify_integrity` | 改ざん検知: 増分更新、署名後の変更、**DocMDP（証明署名）の権限違反を P の値ごとに判定**（下記）、および**リビジョン間のオブジェクト単位差分**（各更新が何を追加・書き換え・削除したか）。増分更新は PDF で合法なので、差分は「何を見るべきか」を示すだけで **verdict は動かさない**。そのオブジェクトがページのどこにあるかは [pdf-reader-mcp](https://github.com/shuji-bonji/pdf-reader-mcp) の `locate_objects` |
...
## DocMDP（証明署名）の権限判定（v0.14）

証明署名の `P` は「**どんな変更を許すか**」を定めます（ISO 32000-2 **Table 257**）。
`verify_integrity` は署名後の変更をオブジェクト単位で分類し、その `P` が許す種類と突き合わせます。

| P | 許される変更 | 例: 注釈の追加は |
|---|---|---|
| 1 | 何も許さない（DSS / 文書タイムスタンプの増分更新は §12.8.2.2 の例外） | **違反** |
| 2 | フォームへの記入・ページテンプレートの実体化・署名 | **違反**（注釈は 3 から） |
| 3 | 2 に加えて注釈の作成・削除・変更 | 違反でない |

許される変更に**必ず付随して動くもの**（`/Annots` が増えたページ・カタログ・`/Info`・XMP）は
`housekeeping` として違反に数えません。これを違反に数えると、正当な P=3 の注釈追加でも
これらが必ず動くため、**すべての証明署名付き文書が違反になります**。

### `violationAssessment` は 3 値です

| 値 | 意味 |
|---|---|
| `permitted` | 署名後の変更はすべて `P` が許す種類だった |
| `violated` | `P` が許さない変更が 1 つ以上あった |
| `indeterminate` | **判定できなかった**（xref チェーンを辿れない／変更されたオブジェクトの種類が読めない） |

> ⚠️ **`indeterminate` は「問題なし」ではありません。** このサーバは**反証する**側であり、
> 「反証できなかった」と「問題ない」は別の主張です（`validate_clauses` が
> `needs_external_fact` を返すのと同じ規律）。
>
> 互換のため残している `violatedByLaterChanges`（boolean）は
> **`indeterminate` を `false` に潰します**。「判定できなかった」を
> 「問題なし」と読み違えてはいけない場面では、必ず `violationAssessment` を読んでください。
> `evaluate_policy` は `indeterminate` でも `human_review_required` に上げます。

なお `/Type` を持たない裸のストリームは `content` ではなく **判定不能**として扱います ——
...
PDF/A-4（`pdfa-4` / `pdfa-4e` / `pdfa-4f`）も指定できます。**PDF/A-4 は conformance レベルを持たない**ため `pdfa-4b` は存在せず、`e` / `f` はレベルではなく variant です。ネイティブ規則は ISO 19005-1/-2 から起こしたもので ISO 19005-4 で検証していないため、PDF/A-4 のレポートには「native の判定は veraPDF に劣後する」と明記されます。part 4 は veraPDF で検証してください。
